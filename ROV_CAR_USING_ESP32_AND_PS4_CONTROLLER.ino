...
void notify()
{
  int rx =(PS4.data.analog.stick.rx);  //Base       =>  Right stick - x axis
  int ry =(PS4.data.analog.stick.ry);  //Shoulder   =>  Right stick  - y axis
  int ly =(PS4.data.analog.stick.ly);  //Elbow      =>  Left stick  - y axis 
  int lx =(PS4.data.analog.stick.lx);  //Gripper    =>  Left stick - x axis
  ...
  if (PS4.data.button.up)             //Move car Forward
  {
    rotateMotor(MAX_MOTOR_SPEED, MAX_MOTOR_SPEED);
  }
  else if (PS4.data.button.down)      //Move car Backward
  {
    rotateMotor(-MAX_MOTOR_SPEED, -MAX_MOTOR_SPEED);
  }
  else if (PS4.data.button.right)     //Move car Right
  {
    rotateMotor(-MAX_MOTOR_SPEED, MAX_MOTOR_SPEED);
  }
  else if (PS4.data.button.left)      //Move car Left
  {
    rotateMotor(MAX_MOTOR_SPEED, -MAX_MOTOR_SPEED);
  }
  else                                //Stop the car
